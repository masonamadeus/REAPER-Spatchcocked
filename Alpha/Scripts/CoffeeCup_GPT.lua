...
--- Set variables
tooLoud = 30 -- set maximum allowed volume increase (in dB)
shortLen = 2 
fadeLen = 0.15
...
function wait_for_change()
    ...
    if new_selected_items > selected_items then
        ...
        for _, itemO in ipairs(overLoud) do
            local groupedItems = {}
            local groupId = reaper.GetMediaItemInfo_Value(itemO,"I_GROUPID")
            if groupId ~= 0 then
              local numItems = reaper.CountMediaItems(0)
              for i=0, numItems-1 do
                local checkItem = reaper.GetMediaItem(0,i)
                if reaper.GetMediaItemInfo_Value(checkItem,"I_GROUPID") == groupId then
                  if checkItem ~= itemO then
                    table.insert(groupedItems,checkItem)
                  end
                end
              end
            end
            
            for _,itemX in ipairs(groupedItems) do
              reaper.DeleteTrackMediaItem(reaper.GetMediaItem_Track(itemX), itemX)
            end
            --reaper.ShowConsoleMsg("deleted loud item")
        end--]]
